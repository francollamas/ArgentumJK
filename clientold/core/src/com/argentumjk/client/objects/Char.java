...
import com.argentumjk.client.graphics.DrawParameter;
import com.argentumjk.client.graphics.Drawer;
import com.argentumjk.client.graphics.FontParameter;
import com.argentumjk.client.graphics.Grh;
import com.argentumjk.client.utils.Position;

import static com.argentumjk.client.general.Config.*;
...
public class Char {
    ...
    public void draw(Batch batch, float x, float y, DrawParameter dp) {
            ...
            if (getMoveDir().getX() != 0 || getMoveDir().getY() != 0) {
                ...
                if (getMoveDir().getX() != 0) {
                    getMoveOffset().setX(getMoveOffset().getX()
                            + Game.getInstance().getConfig().getScrollPixelsPerFrame() * getMoveDir().getX() * Drawer.getDelta());
                ...
                if (getMoveDir().getY() != 0) {
                    getMoveOffset().setY(getMoveOffset().getY()
                            + Game.getInstance().getConfig().getScrollPixelsPerFrame() * getMoveDir().getY() * Drawer.getDelta());
        ...
        Position headOffset;
        if (getBody() != null)
            headOffset = Game.getInstance().getAssets().getBodies().getBody(getBodyIndex()).getHeadOffset();
        else
        ...
        if (!isInvisible()) {
            ...
            if (getNombre().length() > 0) {
                ...
                fp.setColor(Game.getInstance().getGameData().getColors().getColor(getPriv(), getBando()));
        ...
        if (getFxIndex() != 0) {
            Fx fxData = Game.getInstance().getAssets().getFxs().getFx(getFxIndex());
    ...
    public void drawDialog(Batch batch, float x, float y, DrawParameter dp) {
        ...
        if (dialog != null) {
            if (nombre.length() == 0 && this != Game.getInstance().getGameData().getChars().getNpcDialog())
                dialog = null;
            else {
                if (dialog.isAlive()) {
    ...
    public void setFx(int index, int loops) {
        Fx fx = Game.getInstance().getAssets().getFxs().getFx(index);
        if (fx != null) {
            this.fx = fx.getGrh();
    ...
    public void setBody(int index) {
        Body body = Game.getInstance().getAssets().getBodies().getBody(index);
        if (body != null) {
    ...
    public void setHead(int index) {
        GrhDir head = Game.getInstance().getAssets().getHeads().getGrhDir(index);
        if (head != null) {
    ...
    public void setHelmet(int index) {
        GrhDir helmet = Game.getInstance().getAssets().getHelmets().getGrhDir(index);
        if (helmet != null)
    ...
    public void setWeapon(int index) {
        GrhDir weapon = Game.getInstance().getAssets().getWeapons().getGrhDir(index);
        if (weapon != null)
    ...
    public void setShield(int index) {
        GrhDir shield = Game.getInstance().getAssets().getShields().getGrhDir(index);
        if (shield != null)
