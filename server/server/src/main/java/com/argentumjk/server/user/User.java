...
import com.argentumjk.server.util.*;
...
public class User extends AbstractCharacter {
	...
	public void setChannel(Channel channel) {
		...
		if (channel != null) {
			...
			if (addr != null) {
				...
				Gdx.app.log("Info: ", this.userName + " conectado desde " + this.ip);
			}
		}
	}
	...
	public synchronized void sendPacket(ServerPacket packet) {
		...
		try {
			Gdx.app.debug("Debug: ", ">>" + this.userName + ">> " + packet.id());
			Gson gson = new Gson();
		...
		try {
			...
			if (wasLogged) {
				try {
					...
					Gdx.app.error("Fatal: ", "ERROR EN doSALIR() - saveUser(): ", ex);
				}
			}
		}
		Gdx.app.log("Info: ", "Salió: " + getUserName());
	...
	private boolean enterIntoMap(short mapNumber, byte x, byte y, boolean withFX, boolean sendingData) {
		...
		if (pos().map != 0) {
			...
			if (oldMap.hasUser(this) && !oldMap.exitMap(this)) {
				...
				Gdx.app.error("Fatal: ", this.getUserName() + " no pudo salir del mapa actual");
			}
		}

		Map targetMap = this.server.getMap(mapNumber);
		if (targetMap == null) {
			Gdx.app.log("Warn: ", "No existe el mapa " + mapNumber);
			return false;
		}
	...
	public boolean warpMe(short targetMap, byte x, byte y, boolean withFX) {
		...
		if (freePos == null) {
			Gdx.app.log("Warn: ", "WARPUSER FALLO: no hay un lugar libre cerca de mapa=" + targetMap + " x=" + x + " y=" + y);
			return false;
		}
...
//		}
	...
	public void usuarioAtaca() {
		if (getCounters().intervaloPermiteAtacar()) {
			...
			Gdx.app.log("Info: ", "NO PUEDE ATACAR");
		}
	}
	...
	public void actStats(User victima) {
		...
		Gdx.app.log("Info: ", "ASESINATO: " + this.userName + " asesino a " + victima.userName);
	}
...
//				chatColor = Color.rgb(255, 128, 64);
			...
			Gdx.app.error("Fatal: ", "ERROR EN connectUser(), nick=" + this.userName, e);
	...
	public static void changeGuildLeaderChr(String userName, boolean isLeader) {
		try {
			...
			Gdx.app.error("Fatal: ", userName + ": ERROR EN SAVEUSER()", e);
		}
	}
