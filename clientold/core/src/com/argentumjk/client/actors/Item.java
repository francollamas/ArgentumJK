...
import com.argentumjk.client.utils.Actors;

import static com.argentumjk.client.containers.GameData.ObjTypes.*;
...
public class Item extends Slot {
    ...
    public void set(int obj, String nombre, int cant, boolean equip, int grh, int tipo,
                    int maxHit, int minHit, int defense, float precio) {
        this.obj = obj;
        setNombre(nombre);
        setCantidad(cant);
        setEquipado(equip);
        ...
        if (getCantidad() > 0) {
            ...
            if (this.tipo == Armadura || this.tipo == Casco || this.tipo == Escudo) {
                ...
                Actors.newLabel(t, getDefense() + "", Color.WHITE, "smallgradient");
                tt.add(t);
            }
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        float x = getX();
        float y = Game.getInstance().getConfig().getVirtualHeight() - getY() - getHeight();
