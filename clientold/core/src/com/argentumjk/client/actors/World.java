...
import com.argentumjk.client.listeners.ClickLongPressListener;
...
public class World extends Actor {
            ...
            public void tUp(InputEvent event, float x, float y, int pointer, int button) {
                if (button == 0) {
                    if (getTapCount() == 1)
                        ...
                        getClPack().writeWarpChar("YO", Game.getInstance().getGameData().getCurrentUser().getMap(), mouseTile);
            }
        };
    ...
    public Map getMapa() {
        return Game.getInstance().getAssets().getMapa();
    }
    ...
    public void moveChar(Direccion dir) {
        if (Game.getInstance().getGameData().isPausa()) return;
        ...
        if (getMapa().isLegalPos(absPos) && !u.isParalizado() && !u.isDescansando() && !u.isMeditando()) {
            ...
            Game.getInstance().getGameData().getChars().moveChar(u.getIndexInServer(), dir);
            setMove(relPos, absPos);
        }
        else {
            Char c = Game.getInstance().getGameData().getChars().getChar(u.getIndexInServer());
        ...
        for (y = (int)screenBigTile.getY1(); y <= (int)screenBigTile.getY2(); y++) {
            ...
            for (x = (int)screenBigTile.getX1(); x <= (int)screenBigTile.getX2(); x++) {
                ...
                if (tile.getCharIndex() != 0)
                    Game.getInstance().getGameData().getChars().getChar(tile.getCharIndex())
