...
import com.argentumjk.client.objects.Char;
import com.argentumjk.client.objects.MapTile;
...
public class Chars {
    ...
    public void moveChar(int index, int x, int y) {
        ...
        c.setHeading(dir);
        c.getMoveOffset().setX(-1 * Game.getInstance().getConfig().getTilePixelWidth() * xDif);
        c.getMoveOffset().setY(-1 * Game.getInstance().getConfig().getTilePixelHeight() * yDif);
        ...
        Rect area = Game.getInstance().getGameData().getCurrentUser().getArea();
     ...
     */
    public void moveChar(int index, Direccion dir) {
        Char c = chars[index];
        ...
        Game.getInstance().getAssets().getMapa().getTile((int)c.getPos().getX(), (int)c.getPos().getY()).setCharIndex(0);
        Game.getInstance().getAssets().getMapa().getTile((int)absPos.getX(), (int)absPos.getY()).setCharIndex(index);
        c.setPos(absPos);

        c.setHeading(dir);
        c.getMoveOffset().setX(-1 * Game.getInstance().getConfig().getTilePixelWidth() * relPos.getX());
        c.getMoveOffset().setY(-1 * Game.getInstance().getConfig().getTilePixelHeight() * relPos.getY());
        ...
        if (!area.isPointIn(absPos))
            if (index != Game.getInstance().getGameData().getCurrentUser().getIndexInServer())
                deleteChar(index);
    }
    ...
    public void deleteChar(int index) {
        ...
        // Lo saco del mapa
        MapTile tile = Game.getInstance().getAssets().getMapa().getTile((int)c.getPos().getX(), (int)c.getPos().getY());
        tile.setCharIndex(0);
     ...
     */
    public void clear() {
        int lastChar = Game.getInstance().getGameData().getChars().getLastChar();
        for (int i = 1; i <=lastChar; i++) {
            if (Game.getInstance().getGameData().getChars().getChar(i) != null)
                deleteChar(i);
        }
    }
    ...
    public void refresh() {
        int lastChar = Game.getInstance().getGameData().getChars().getLastChar();
        for (int i = 1; i <=lastChar; i++) {
            Char c = Game.getInstance().getGameData().getChars().getChar(i);
            if (c != null && c.isActive()) {
                Game.getInstance().getAssets().getMapa().getTile((int)c.getPos().getX(), (int)c.getPos().getY()).setCharIndex(i);
            }
        }
    }
