 ...
 *******************************************************************************/
package com.argentumjk.server.quest;
...
public class Quest implements Constants {
	...
	public void load(IniFile ini) {
		...
		for (short j = 0; j < CANT_NPCS; j++) {
			...
			try {
				...
				Gdx.app.error("Fatal: ", "Error en cargarQuests(): quest=" + this.nroQuest + " npc=#" + (j + 1), e);
			}
		}
	...
	public void hacerQuest(User user, Npc npc) {
		try {
			...
			Gdx.app.error("Fatal: ", user.getUserName() + " Error en HacerQuest!", e);
		}
	}
	...
	public void recibirRecompensaQuest(User user) {
		try {
			...
			Gdx.app.error("Fatal: ", user.getUserName() + " Error en RecibirRecompensaQuest!", e);
		}
	}
	...
	public void userSeRinde(User user) {
		...
		try {
			...
			Gdx.app.error("Fatal: ", user.getUserName() + " Error en UserSeRinde!", e);
		}
	}
