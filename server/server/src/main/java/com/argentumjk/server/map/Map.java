...
import com.argentumjk.server.protocol.CharacterMoveResponse;
import com.argentumjk.server.protocol.CharacterRemoveResponse;
import com.argentumjk.server.protocol.CreateFXResponse;
import com.argentumjk.server.protocol.ForceCharMoveResponse;
import com.argentumjk.server.protocol.ObjectCreateResponse;
import com.argentumjk.server.protocol.ObjectDeleteResponse;
import com.argentumjk.server.protocol.PlayWaveResponse;
import com.argentumjk.server.protocol.RemoveCharDialogResponse;
import com.argentumjk.server.protocol.ShowSignalResponse;
import com.argentumjk.server.user.User;
import com.argentumjk.server.util.BytesReader;
...
public class Map implements Constants {
    ...
    final static String FOLDER_MAPS = "svmaps";
    ...
    throws java.io.FileNotFoundException, java.io.IOException {
        ...
        // Cargar informaciï¿½n del archivo .dat
        ...
        // User Kiling estï¿½ invertido 0 habilita PK, 1 deshabilita PK.
        ...
        // FIXME esta informaciï¿½n sigue estando en los mapas?
    ...
    throws java.io.IOException {
	    ...
	    try {
                ...
                for (int x = 0; x < MAP_WIDTH; x++) {
                    ...
                    if ((byflags & 16) == 16) {
                    	...
                    	if (value > Trigger.values().length-1 || value < 0) {
                    		Gdx.app.log("Warn: ", "Trigger fuera de rango: " + value + " en mapa:" + mapFileName + " x:" + (x+1) + " y:" +(y+1));
                    	} else {
        	...
        	Gdx.app.error("Fatal: ", "ERROR LOADING " + mapFileName, e);
        }
    }
    ...
    throws java.io.IOException {
        ...
        if (!loadBackup || !fileHandle.exists()) {
        	...
        	if (!fileHandle.exists()) {
        		Gdx.app.error("Error: ", "Error, archivo de mapa inexistente: " + infFileName);
        		return;
        	}
        }
        ...
        try {
        	...
        	Gdx.app.error("Error: ", "Error, cargando " + infFileName, e);
        }
    }
    ...
    public boolean exitMap(User user) {
        ...
        try {
	        ...
	        if (tile(x,y).userId() != user.getId()) {
	        	Gdx.app.error("Fatal: ", "INCONSISTENCIA: el jugador no se encuentra donde deberï¿½a");
	            return false;
	        }
    ...
    public void moveNpc(Npc npc, byte x, byte y) {
        if (tile(x, y).npc() != null) {
			Gdx.app.error("Fatal: ", "ERRRRRRRRRRORRRRRRRRRRRR en moverNpc: " + npc);
		}
    ...
    public void removeNpcsArea(User admin) {
        ...
        for (short y = y1; y <= y2; y++) {
            for (short x = x1; x <= x2; x++) {
            	npc = tile(x,y).npc();
            	// gorlok: por seguridad, lo limito a sï¿½lo NPC hostiles que no sean mascotas
    ...
    public boolean addObject(short objid, int cant, byte x, byte y) {
    	if (hasObject(x, y)) {
            Gdx.app.log("Warn: ", "Intento de agregar objeto sobre otro: objid=" + objid + " cant" + cant + " mapa" + this.mapNumber + " x=" + x + " y=" + y);
    		return false;
    	}
    ...
    public void lookAtTile(User user, byte  x, byte y) {
        ...
        if ((npc = lookForNearbyNpc(x, y)) != null) {
            ...
            if (npc.description.length() > 0) {
                ...
                // enviamos nuevamente el mensaje del Centinela, segï¿½n quien pregunta.
            	...
            	if (user.getFlags().isGM() && npc.attackedFirstBy != "") {
            		user.sendMessage("Le pegï¿½ primero: " + npc.attackedFirstBy + ".", FontType.FONTTYPE_INFO);
            	}
            }
    ...
    public void accionParaRamita(byte x, byte y, User user) {
        if (Util.distance(user.pos().x, user.pos().y, x, y) > 2) {
            user.sendMessage("Estï¿½s demasiado lejos.", FontType.FONTTYPE_INFO);
            return;
        }
    ...
    public MapPos dropItemOnFloor(byte  x, byte y, InventoryObject obj) {
        ...
        if (newPos != null) {
	        Gdx.app.debug("Debug: ", "tirarItemAlPiso: x=" + newPos.x + " y=" + newPos.y);
	        if (newPos != null) {
    ...
    private boolean isFreePosForUser(byte  x, byte y, boolean sailing, boolean isAdmin) {
    	if (isAdmin) {
			return isFreePosForAdmin(x, y); // Los Admins no respetan las leyes de la fï¿½sica :P
    ...
    /** 
     * Busca una posicion vï¿½lida para un jugador, y que sea lo mï¿½s cercana a la posiciï¿½n original 
     */
    ...
    private boolean isFreePosForNpc(byte  x, byte y, boolean canWater, boolean canLand, boolean underRoof) {
    	if (isWater(x, y)) {
            ...
            // Comprobamos si es una posiciï¿½n libre sin agua
            if (!canLand) {
				return isFreePosWithoutWater(x, y) 
						&& !isTeleport(x, y) 
						&& !hasObject(x, y) 
    ...
    /** 
     * Busca una posicion vï¿½lida para un Npc, y que sea lo mï¿½s cercana a la posiciï¿½n original 
     */
    ...
    private boolean isFreePosForAdmin(byte  x, byte y) {
        // Los Admins no respetan las leyes de la fï¿½sica :P
        return Pos.isValid(x, y) 
    ...
    public void sendCreaturesInMap(User admin) {
        ...
        for (Npc npc : this.npcs) {
            ...
            if (npc.isNpcActive() && npc.isHostile() && npc.stats.alineacion == 2) {
            	// Ya estaba este nï¿½mero de NPC en la lista?
    ...
    public void saveDatFile() {
        ...
        try {
            ...
            Gdx.app.error("Fatal: ", "ERROR GUARDANDO MAPA " + this.mapNumber, e);
        }
    }
    ...
    private void saveMapFile() {
        ...
        Gdx.app.log("Info: ", "Guardando mapa: " + filename);
        try {
            DataOutputStream f =
            ...
            Gdx.app.error("Fatal: ", "ERROR EN saveMapFile " + this.mapNumber, e);
        }
    }
    ...
    private void saveInfFile() {
        ...
        try {
            ...
            Gdx.app.error("Fatal: ", "ERROR EN saveInfFile " + this.mapNumber, e);
        }
    }
    ...
    public void doTileEvents(User user) {
    	...
    	// para teletransportarlo, desde la funciï¿½n mover disparamos el TP directamente.
    }
    ...
    public boolean isForbbidenMap(User user) {
    	// ï¿½Es mapa de newbies?
    	if (isNewbieMap()) {
    		if (user.isNewbie() || user.getFlags().isGM()) {
    			return false; // allowed
    		} else {
    			// no es un newbie/gm, "NO PASARï¿½S!"
				user.sendMessage("Mapa exclusivo para newbies.", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	} 
    	
		// ï¿½Es mapa de Armadas?
    	if (isRoyalArmyMap()) {
            // ï¿½El usuario es Armada?
    		if (user.isRoyalArmy() || user.getFlags().isGM()) {
    			return false; // allowed
    		} else {
    			// no es un armada/gm, "NO PASARï¿½S!"
				user.sendMessage("Mapa exclusivo para miembros del ejï¿½rcito Real", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
		...
		// ï¿½Es mapa de Caos?
    	if (isDarkLegionMap()) {
            // ï¿½El usuario es Caos?
    		if (user.isDarkLegion() || user.getFlags().isGM()) {
    			return false; // allowed
    		} else {
    			// no es un caos/gm, "NO PASARï¿½S!"
				user.sendMessage("Mapa exclusivo para miembros del ejï¿½rcito Oscuro.", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
    	
		// ï¿½Es mapa de faccionarios?
    	if (isFactionMap()) {
            // ï¿½El usuario es Caos?
    		if (user.isRoyalArmy() || user.isDarkLegion() || user.getFlags().isGM()) {
    			return false; // allowed
    		} else {
    			// no es un armada/caos/gm, "NO PASARï¿½S!"
				user.sendMessage("Solo se permite entrar al Mapa si eres miembro de alguna Facciï¿½n", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
