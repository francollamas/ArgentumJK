...
public class Map implements Constants {
    ...
    throws java.io.FileNotFoundException, java.io.IOException {
        ...
        // User Kiling est� invertido 0 habilita PK, 1 deshabilita PK.
        ...
        // FIXME esta informaci�n sigue estando en los mapas?
    ...
    public boolean exitMap(User user) {
        ...
        try {
	        ...
	        if (tile(x,y).userId() != user.getId()) {
	        	Gdx.app.error("Fatal: ", "INCONSISTENCIA: el jugador no se encuentra donde deber�a");
	            return false;
	        }
    ...
    public void removeNpcsArea(User admin) {
        ...
        for (short y = y1; y <= y2; y++) {
            for (short x = x1; x <= x2; x++) {
            	npc = tile(x,y).npc();
            	// gorlok: por seguridad, lo limito a s�lo NPC hostiles que no sean mascotas
    ...
    public void lookAtTile(User user, byte  x, byte y) {
        ...
        if ((npc = lookForNearbyNpc(x, y)) != null) {
            ...
            if (npc.description.length() > 0) {
                ...
                // enviamos nuevamente el mensaje del Centinela, seg�n quien pregunta.
            	...
            	if (user.getFlags().isGM() && npc.attackedFirstBy != "") {
            		user.sendMessage("Le peg� primero: " + npc.attackedFirstBy + ".", FontType.FONTTYPE_INFO);
            	}
            }
    ...
    public void accionParaRamita(byte x, byte y, User user) {
        if (Util.distance(user.pos().x, user.pos().y, x, y) > 2) {
            user.sendMessage("Est�s demasiado lejos.", FontType.FONTTYPE_INFO);
            return;
        }
    ...
    private boolean isFreePosForUser(byte  x, byte y, boolean sailing, boolean isAdmin) {
    	if (isAdmin) {
			return isFreePosForAdmin(x, y); // Los Admins no respetan las leyes de la f�sica :P
    ...
    /** 
     * Busca una posicion v�lida para un jugador, y que sea lo m�s cercana a la posici�n original 
     */
    ...
    /** 
     * Busca una posicion v�lida para un Npc, y que sea lo m�s cercana a la posici�n original 
     */
    ...
    public void sendCreaturesInMap(User admin) {
        ...
        for (Npc npc : this.npcs) {
            ...
            if (npc.isNpcActive() && npc.isHostile() && npc.stats.alineacion == 2) {
            	// Ya estaba este n�mero de NPC en la lista?
    ...
    public void doTileEvents(User user) {
    	...
    	// para teletransportarlo, desde la funci�n mover disparamos el TP directamente.
    }
    ...
    public boolean isForbbidenMap(User user) {
    	...
    	if (isRoyalArmyMap()) {
    		...
    		if (user.isRoyalArmy() || user.getFlags().isGM()) {
    			...
    			// no es un armada/gm, "NO PASAR�S!"
				user.sendMessage("Mapa exclusivo para miembros del ej�rcito Real", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
    	...
    	if (isDarkLegionMap()) {
    		...
    		if (user.isDarkLegion() || user.getFlags().isGM()) {
    			...
    			// no es un caos/gm, "NO PASAR�S!"
				user.sendMessage("Mapa exclusivo para miembros del ej�rcito Oscuro.", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
    	...
    	if (isFactionMap()) {
    		...
    		if (user.isRoyalArmy() || user.isDarkLegion() || user.getFlags().isGM()) {
    			...
    			// no es un armada/caos/gm, "NO PASAR�S!"
				user.sendMessage("Solo se permite entrar al Mapa si eres miembro de alguna Facci�n", FontType.FONTTYPE_INFO);
    			return true;
    		}
    	}
